...
export class AuthService {
     ...
     * Update user profile
     */
    updateProfile(data: { username?: string; email?: string; phoneNumber?: string; fullName?: string }): Observable<any> {
        return this.http.put(`${environment.apiUrl}/users/profile`, data).pipe(
            tap((response: any) => {
                if (response.user) {
                    // Update local storage and subject with new user details
                    const currentUser = this.currentUserSubject.value;
                    const updatedUser = { ...currentUser, ...response.user };
                    this.setSession({ ...response, user: updatedUser });
                }
            })
        );
    }
